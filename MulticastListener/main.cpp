...
static const auto MULTICAST_GROUP_ADDRESS = _T("10.159.1.254");
...
int test_multicast_with_local_ip()
{
	...
	InetPton(AF_INET, LOCAL_IP, &localaddr.sin_addr.s_addr);
	...
	InetPton(AF_INET, LOCAL_IP, &ipmr.imr_interface.s_addr);
	InetPton(AF_INET, MULTICAST_GROUP_ADDRESS, &ipmr.imr_multiaddr.s_addr);
	int len = sizeof(ipmr);
	...
	int nbytes = 0;
	InetPton(AF_INET, MULTICAST_GROUP_ADDRESS, &localaddr.sin_addr.s_addr);
